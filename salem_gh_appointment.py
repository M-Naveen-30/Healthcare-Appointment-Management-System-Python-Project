"""
==========================================================================
                SALEM GOVERNMENT HOSPITAL  -  APPOINTMENT SYSTEM
                          Mini Project  (PyCharm runnable)
==========================================================================
Pattern follows the original `shivaminipro.py`  (pygame + recursive
login + pywhatkit).  Everything is kept in ONE file for easy submission.
--------------------------------------------------------------------------
STEPS
 1  Hospital LOGO 5 sec  +  10 sec welcome music
 2  WELCOME banner with stars
 3  Register (Name / Age / Gender / Phone / Password)
 4  Login form (recursive – retries until correct)
 5  4-digit OTP sent to your WhatsApp via pywhatkit (must enter it)
 6  4 doctors group photo for 7 sec
 7  Choose doctor (1-Ponvanan / 2-Vasudha / 3-Sam / 4-Nethra)
 8  Individual doctor photo for 5 sec
 9  Print MAY (current year) calendar
10  Pick a date (manual)
11  Pick time slot - DAY / NOON / EVENING
12  Purpose of visit
13  Allocate token (1 / 2 / 3) - real availability check (Extra-D)
14  PY-Pay UPI QR code shown for 6 sec
15  "Thank You ! Payment Successful"
16  Pillow slip image  +  ReportLab PDF slip (Extra-E)
17  pywhatkit auto-sends slip to your WhatsApp
==========================================================================
"""
...
def show_image(path, caption, seconds, size=(640, 640), with_music=False):
    """Open a pygame window, blit the picture, optionally play music, hold N sec.
    Re-blits every frame and pumps events so the window stays responsive
    (fixes blank/white windows on Windows)."""
    p.init()
    p.display.init()
...
show_image(LOGO_FILE, "SALEM GOVERNMENT HOSPITAL", seconds=10, size=(640, 640),
           with_music=True)
# logo shown 10 sec same as music; the spec said logo 5 sec + music 10 sec,
# we keep music for the full 10 sec but logo stays the whole time so user
# can hear the welcome tune.
...
# --------------------------------------------------------------------------
#  STEP 4  -  LOGIN  (recursive – retries until correct)
# --------------------------------------------------------------------------
print("---------------------------- LOGIN -----------------------------")
...
# --------------------------------------------------------------------------
#  STEP 10  -  PICK DATE  (1 - 31, must be valid May date)
# --------------------------------------------------------------------------
day = 0
while not (1 <= day <= 31):
    try:
...
# --------------------------------------------------------------------------
#  STEP 13  -  TOKEN ALLOCATION   (Extra-D real availability check)
# --------------------------------------------------------------------------
db = load_db()
...
# --------------------------------------------------------------------------
#  STEP 14  -  PY-PAY QR  (real UPI string, 6 sec)
# --------------------------------------------------------------------------
fee = 100
...
show_image(qr_path, "PY-Pay  -  Scan to Pay", seconds=6, size=(W, H))
...
# --------------------------------------------------------------------------
#  STEP 16  -  PILLOW SLIP IMAGE  +  PDF SLIP  (Extra-E)
# --------------------------------------------------------------------------
